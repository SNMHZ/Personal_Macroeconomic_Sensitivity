...
class Preprocessor_MR():
    def __init__(self, base_dataset_file_path):
        ...
        self.m_feats = ['원달러-2', '원달러-1', 'CPI-2', 'CPI-1', '제조업BSI-2', '제조업BSI-1', '비제조업BSI-2', '비제조업BSI-1', '고용율-2', '고용율-1']
        ...
        for label in self.labels:
          self.base_dataset[label] = self.base_dataset[label].astype('float32')

        for col in self.feats1+self.feats2:
          self.winsorizeOutlier(self.base_dataset[col])
        
        self.scaleVector = self.base_dataset[self.feats1 + self.m_feats + self.feats2 + self.feats3].max().values
        del self.base_dataset
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    def makeTimeSeries(self, df: pd.DataFrame):
        if '삼개월급여이체실적금액' not in df.columns:
          return df
        df['급여-2']=(df['삼개월급여이체실적금액']-df['전월급여이체실적금액']-df['급여이체실적금액'])
        df['급여-2'].mask(df['급여-2']<0, 0, inplace=True)
        df['급여-1']=df['전월급여이체실적금액']
    ...
    def scaleInputData(self, df: pd.DataFrame) -> pd.DataFrame:
        return df[self.feats1 + self.m_feats + self.feats2 + self.feats3] / self.scaleVector
    ...
    def load_and_preprocess(self, dataset_file_path):
        ...
        return self.as_tensor(input_data), self.as_tensor(label_data)